#!/usr/bin/env python3
"""
Claude Code Executor - Claude Code CLI execution backend.

This module provides the ClaudeCodeExecutor implementation using claude-agent-sdk
for running Claude Code CLI with multi-agent workflow support.
"""
...
try:
    ...
    SDK_AVAILABLE = False
    logging.getLogger(__name__).warning("claude-agent-sdk not available, falling back to direct CLI call")

# 导入实时状态跟踪
try:
    from ..observability.live_status import LiveStatusTracker, LiveStatusEntry
...
"""
    ...
    def _extract_outstanding_items(self, evaluation_text: str) -> List[str]:
        ...
        for line in lines:
            ...
            if in_outstanding:
                if re.match(r'^\s*[-*❌✓]\s+', line) or re.match(r'^\s*\[\s*[x\s]\]\s+', line):
