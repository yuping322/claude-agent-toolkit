#!/usr/bin/env python3
"""
Bug Fix Agent - Independent agent execution logic for fixing GitHub issues.

This module provides a pluggable interface for different AI agents to fix GitHub issues.
Users can easily replace the underlying agent implementation by implementing the BugFixAgentInterface.
"""
...
def create_bug_fix_agent(agent_type: str = "claude", tools: list[BaseTool] = None, workspace_path: str = None) -> BugFixAgentInterface:
    """
    Factory function to create bug fix agents.

    Args:
        agent_type: Type of agent to create ("claude" for now, extensible for future agents)
    ...
    if agent_type == "claude":
        if tools is None or workspace_path is None:
            raise ValueError("tools and workspace_path are required for Claude agent")
        return ClaudeBugFixAgent(tools, workspace_path)
    else:
